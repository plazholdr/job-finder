...
import { RequestStatus as RS } from '../../constants/enums.js';
...
export default (app) => {
  ...
  async function onCreate(ctx) {
    ...
    const requestedBy = user.role === 'company' ? 'company' : (user.role === 'student' ? 'student' : null);
    if (!requestedBy) throw Object.assign(new Error('Forbidden'), { code: 403 });
    // Ownership check
    if (requestedBy === 'student' && String(emp.userId) !== String(user._id)) throw Object.assign(new Error('Forbidden'), { code: 403 });
    ...
    ctx.data = { employmentId: emp._id, requestedBy, requestedUntil: new Date(body.requestedUntil), reason: body.reason, status: RS.PENDING };
  }
  ...
  async function applyAction(ctx) {
    ...
    const doc = await Extensions.findById(id);
    ...
    if (action === 'cancel' && ((doc.status === RS.PENDING) || !ctx.params.provider)) {
      // Only initiator can cancel while pending
      if ((doc.requestedBy === 'student' && user.role === 'student' && String(emp.userId) === String(user._id)) ||
          (doc.requestedBy === 'company' && user.role === 'company')) {
    ...
    if ((user.role === 'company' || user.role === 'admin') && doc.status === RS.PENDING) {
      if (action === 'approve') {
        // extend endDate
        await Employment.updateOne({ _id: emp._id }, { $set: { endDate: doc.requestedUntil } });

...
export default function InternCompletionPage() {
  ...
  const [selectedIntern, setSelectedIntern] = useState<CompletingIntern | null>(
    null
  );
  ...
  const fetchCompletingInterns = async () => {
    try {
      ...
      const response = await fetch(
        '/api/company/recruitment/interns/completion'
      );
  ...
  const updateCompletionStatus = async (
    ...
    try {
      const response = await fetch(
        `/api/company/recruitment/interns/completion/${internId}/status`,
        {
          method: 'PATCH',
      ...
      if (result.success) {
        ...
        setSuccess(
          `Completion status updated to ${newStatus.replace('_', ' ')}`
        );
      } else {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'documentation_finalization':
  ...
  const getAvailableActions = (status: string) => {
    ...
    > = {
      active: [
        {
          id: 'start_completion',
          label: 'Start Completion Process',
          type: 'positive',
        },
        {
          id: 'extend_internship',
      ...
      project_review: [
        {
          ...
          label: 'Conduct Performance Evaluation',
          type: 'positive',
        },
      ],
      performance_evaluation: [
      ...
      certificate_generation: [
        {
          id: 'enroll_alumni',
          label: 'Enroll in Alumni Network',
          type: 'positive',
        },
      ],
      alumni_network: [
  ...
  const filteredInterns = interns.filter((intern) => {
    const matchesStatus =
      filterStatus === 'all' || intern.completionStatus === filterStatus;
    const matchesSearch =
      searchTerm === '' ||
      intern.personalInfo.name
        .toLowerCase()
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Manage intern completion process and alumni transition
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link href="/company/recruitment/interns">
                <Button variant="outline">Back to Interns</Button>
              </Link>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          ...
          >
            ...
            <option value="documentation_finalization">
              Documentation Finalization
            </option>
            <option value="certificate_generation">
                  ...
                  {filteredInterns.map((intern) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <Badge
                          className={getStatusColor(intern.completionStatus)}
                        >
                      ...
                      <div className="mb-3">
                        ...
                        <Progress
                          value={calculateOverallProgress(intern)}
                          className="h-2"
                        />
                      </div>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                    ...
                    <Progress
                      value={calculateOverallProgress(selectedIntern)}
                      className="h-3"
                    />
                  </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex items-center">
                            ...
                            {selectedIntern.personalInfo.major} -{' '}
                            {selectedIntern.personalInfo.year}
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2 text-sm">
                          <div>
                            ...
                            {selectedIntern.internshipDetails.position}
                          </div>
                          <div>
                            <span className="font-medium">Department:</span>{' '}
                            {selectedIntern.internshipDetails.department}
                          </div>
                          <div>
                            <span className="font-medium">Duration:</span>{' '}
                            {selectedIntern.internshipDetails.duration} months
                          </div>
                          <div>
                            <span className="font-medium">Start Date:</span>{' '}
                            {formatDate(
                              selectedIntern.internshipDetails.startDate
                            )}
                          </div>
                          <div>
                            <span className="font-medium">End Date:</span>{' '}
                            {formatDate(
                              selectedIntern.internshipDetails.endDate
                            )}
                          </div>
                          <div>
                            <span className="font-medium">Mentor:</span>{' '}
                            {selectedIntern.internshipDetails.mentor}
                          </div>
                          <div>
                            <span className="font-medium">Supervisor:</span>{' '}
                            {selectedIntern.internshipDetails.supervisor}
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {selectedIntern.internshipDetails.projects.map(
                            (project) => (
                              <Badge
                                key={project}
                                variant="outline"
                          ...
                          {[
                            {
                              ...
                              completed:
                                selectedIntern.completionProgress.projectReview
                                  .completed,
                            },
                            {
                              ...
                              completed:
                                selectedIntern.completionProgress
                            ...
                            },
                            {
                              ...
                              completed:
                                selectedIntern.completionProgress.exitInterview
                                  .completed,
                            },
                            {
                              key: 'documentationFinalization',
                              label: 'Documentation Finalization',
                              completed:
                                selectedIntern.completionProgress
                                  .documentationFinalization.completed,
                            },
                            {
                              ...
                              completed:
                                selectedIntern.completionProgress
                                  .certificateGeneration.completed,
                            },
                            {
                              key: 'alumniNetwork',
                              label: 'Alumni Network Enrollment',
                              completed:
                                selectedIntern.completionProgress.alumniNetwork
                                  .completed,
                            },
                          ].map((item) => (
                            <div
                  ...
                  {activeTab === 'projects' && (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-4">
                        {selectedIntern.completionProgress.projectReview.projects.map(
                          (project, index) => (
                                  ...
                                  {project.reviewedBy &&
                                    project.reviewedDate && (
                                      <p className="text-xs text-gray-500 mt-1">
                                        Reviewed by {project.reviewedBy} on{' '}
                                        {formatDate(project.reviewedDate)}
                                      </p>
                                    )}
                                </div>
                              )}
                  ...
                  {activeTab === 'evaluation' && (
                    <div className="space-y-4">
                      ...
                      {selectedIntern.completionProgress.performanceEvaluation.evaluations.map(
                        (evaluation, index) => (
                          <div key={index} className="p-4 border rounded-lg">
                            <div className="flex items-center justify-between mb-3">
                              <div>
                      ...
                      {selectedIntern.completionProgress.performanceEvaluation
                        .selfEvaluation && (
                          ...
                          <div className="space-y-3 text-sm">
                            <div>
                              ...
                              <ul className="list-disc list-inside text-gray-600">
                                {selectedIntern.completionProgress.performanceEvaluation.selfEvaluation.learnings.map(
                                  (learning, index) => (
                            ...
                            <div>
                              ...
                              <ul className="list-disc list-inside text-gray-600">
                                {selectedIntern.completionProgress.performanceEvaluation.selfEvaluation.achievements.map(
                                  (achievement, index) => (
                            ...
                            <div>
                              ...
                              <ul className="list-disc list-inside text-gray-600">
                                {selectedIntern.completionProgress.performanceEvaluation.selfEvaluation.careerGoals.map(
                                  (goal, index) => (
                  ...
                  <div className="mt-6 pt-4 border-t">
                    ...
                    <div className="space-y-2">
                      {getAvailableActions(selectedIntern.completionStatus).map(
                        (action) => (
                          <Button
                            key={action.id}
                            variant={
              ...
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-gray-600">
                    Choose an intern from the list to view completion details
                    and manage their transition process
                  </p>
                </CardContent>
              </Card>
            )}
